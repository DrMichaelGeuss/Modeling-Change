...
cusp_equilibria <- function(alpha, beta, tol = 1e-8) {
  ...
  Re(roots)[abs(Im(roots)) < tol]
}

classify_equilibria <- function(z, alpha, beta) {
  data.frame(
    z = z,
    curvature = V_second(z, beta),
    type = ifelse(V_second(z, beta) > 0, "stable", "unstable")
  )
}

dX <- function(x, y, a0, a1, a2) a0 + a1 * x + a2 * y
dY <- function(x, y, b0, b1, b2) b0 + b1 * x + b2 * y
